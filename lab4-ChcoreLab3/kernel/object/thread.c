...
void create_root_thread(void)
{
        ...
        for (int i = 0; i < meta.phnum; i++) {
                ...
                /* LAB 3 TODO BEGIN */
                // How to define the phs addr is still a problem! virt_to_phys((void *) vaddr)
                ret = create_pmo(memsz, PMO_DATA, root_cap_group, 0 , &segment_pmo);
                /* LAB 3 TODO END */
                ...
                memcpy((void *)phys_to_virt(segment_pmo->start) + (vaddr & OFFSET_MASK), 
                       (void *)((unsigned long)&binary_procmgr_bin_start + ROOT_PHDR_OFF +  offset + meta.phnum * ROOT_PHENT_SIZE), 
                       filesz);
                ...
                /* LAB 3 TODO BEGIN */
                /* Set flags*/
                vmr_flags = ((flags & PHDR_FLAGS_R) ? VMR_READ  : 0)
                          | ((flags & PHDR_FLAGS_W) ? VMR_WRITE : 0)
                          | ((flags & PHDR_FLAGS_X) ? VMR_EXEC  : 0);
